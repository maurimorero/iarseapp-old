...
from models import Question, Category, Survey, Response, AnswerText, AnswerRadio, AnswerSelect, AnswerInteger, \
    AnswerSelectMultiple, ResponseMgr
...
class ResponseForm(models.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        responseMgr = kwargs.pop('responseMgr')
        self.responseMgr=responseMgr
        self.user=usuario
        self.survey = survey
        ...
        for q in survey.questions():
            ...
            if q.category:
                ...
                self.fields["question_%d" % q.pk].widget.attrs["category"] = q.category
                self.fields["question_%d" % q.pk].widget.attrs["onclick"] = "metodo(this.id)"
    ...
    def save(self, commit=True):
        ...
        response.respMgr=self.responseMgr
        response.interview_uuid = self.uuid
        response.completada=True
        response.save()
...
class ResponseMgrForm(forms.ModelForm):
    class Meta:
        model = ResponseMgr
        fields = [
            'comentarios',
        ...
        labels = {
            'comentarios':'Comentario sobre el proceso de respuesta (opcional)',
        }